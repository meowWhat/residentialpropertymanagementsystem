...
{
  name: '员工管理',
  message: ['员工id', '姓名', '账号', '密码'],
  messages: []
},
{
  name: '用户管理',
  message: ['角色ID', '业主ID', '昵称', '账号', '密码', '权限'],
  messages: []
}
];
  ...
  $('li').click(function () {
    ...
    switch (index) {
      ...
      case 6: //请求物资管理信息
        ...
        break;
      case 7: //请求员工管理
        dataHandleFinal('/getData/emp', index);
        break;
      case 8: //请求用户管理信息
        $.post('/getData/user', {
          index: index
    ...
    .on('click', '#admin_add', function () { //数据添加
      ...
      switch (index) {
        ...
        case 6: //添加物资管理信息
          ...
          break;
        case 7: //添加员工管理
          insert('/insertData/emp', arrs);
          break;
        case 8: //添加员工管理
          insert('/insertData/user', arrs);
          break;
      }
    ...
    .on('click', '.admin_remove', function () { //删除数据操作
      ...
      switch (index) {
        ...
        case 6:
          ...
          break;
        case 7:
          remove('/removeData/emp', data);
          break;
        case 8:
          mdalshow('用户信息无法删除，要删除请去业主管理 。。删完就全删了!!!');
          break;
      }
    })
    .on('click', '#k_search', function () { // 模糊查询 请求
      switch (index) {
        ...
        case 6: //请求物资管理信息
          ...
          break;
        case 7: //请求员工管理
          dataHandleFinal('/getData/emp/key', $('#k_text').val());
          break;
        case 8: //请求用户管理模糊搜索
          $.post('/getData/user/key', {
            index: $('#k_text').val()
          }, function (data, err) {
            if (data.length > 0) {
              user[index].messages = [];
    ...
    .on('click', '#update', function () { // 保存按钮点击发送数据修改请求
      ...
      switch (index) {
        ...
        case 7:
          ...
          break;
        case 8: //这里有坑
          $.post('/updateData/user', {
            data: data
          }, function (data, err) {
            if (data.status == 200)
              return mdalshow('恭喜您修改成功 自动刷新');
            mdalshow('只能修改密码,权限,昵称 自动刷新');
          });
          break;
      }
    });
